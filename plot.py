...
class Plot:
    ...
    def display_waveform(self, core):
        ...
        beat_samples = [(beat / 1000) * self.core.sample_rate for beat in self.core.beats]

        # Adjust for downsampling
        beat_samples_downsampled = [beat / self.plot_downsample for beat in beat_samples]

        # Set the initial tick locations to the beats
        self.beat_axis.set_xticks(beat_samples_downsampled)
        self.beat_axis.set_xticklabels([''] * len(beat_samples_downsampled))  # No labels, just ticks
    ...
    def on_select(self, xmin, xmax):
        ...
        self.core.on_select(xmin * self.plot_downsample, xmax* self.plot_downsample)
    ...
    def update_plot(self):
        if self.plot_data is not None:
            ...
            self.current_plot_pos = int(current_time / 1000 * self.core.sample_rate / self.plot_downsample)
    ...
    def update_xaxis_labels(self):
        ...
        formatter = ticker.FuncFormatter(lambda x, pos: time.strftime('%M:%S', time.gmtime(x / self.core.sample_rate * self.plot_downsample)))
    ...
    def update_beat_axis(self):
        ...
        if hasattr(self, 'beat_axis'):
            ...
            beat_samples_downsampled = [(beat / 1000) * self.core.sample_rate / self.plot_downsample for beat in self.core.beats]
            self.beat_axis.set_xticks(beat_samples_downsampled)
            self.beat_axis.set_xticklabels([''] * len(beat_samples_downsampled))
